...
const owner = (request: Request): string => {
  const value = header(request, "x-owner-account-id");
  ...
  async list(
    ...
    return {
      ...
      page: {
        ...
        ...(result.hasMore && result.rows.at(-1)
          ? { nextCursor: result.rows.at(-1)!._id }
          : {}),
      },
    };
  }
...
@Module({
  ...
  providers: [
    JournalService,
    {
      ...
      useFactory: () => new MongoJournalStore(loadConfiguration()),
    },
  ],
})

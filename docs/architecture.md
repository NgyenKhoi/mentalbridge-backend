...
7. Failure produces a terminal job state after the bounded retry; the user can still read the journal. A reproducible synthetic OpenAI/Gemini benchmark gates each approval identifier and official controlled-demo enablement; adapters and harnesses alone do not constitute approval.

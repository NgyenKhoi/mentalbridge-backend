...
- Raw idempotency keys are never persisted. The service stores a keyed `keyHash` and request `fingerprint`; an exact retry returns the recorded result while reuse with different input is rejected with `409`.
- `commands` is bounded to 32 records per entry. The owner plus command-key index makes command replay unique within the authenticated owner boundary.
...
- A new edit creates a revision and invalidates prior "current analysis"; it does not overwrite provenance. Revise and delete use one atomic filter over owner, journal ID, active state, expected revision, and unseen command hash; a stale `If-Match` is rejected with `412`.

...
| [Care support routing and SupportPlan](care-support-and-intervention-policy.md) | `1.4-scope-v2` | Controlled Capstone v1 routing published; scope v2 product policy approved; v2 runtime unavailable | Keeps v1 routing immutable while adding both safety triggers, all-tier Support Guide, and paid single official SupportPlan authority |
...
| [SupportPlan policy v2](support-plan-policy-v2.md) | `MB-SCOPE-V2-001` | Product policy approved; runtime not implemented | Distinguishes the all-tier Support Guide from the `PLUS`/`PREMIUM` durable plan and governs specialist `PlanChangeRequest` input |

...
12. **Write feature-first Jira work.** A new feature Story must say what usable capability exists when it is done: concrete owner APIs, frontend surface and current frontend gap, user/actor flow, persisted outcome where applicable, and runnable acceptance evidence. Include one early subtask to confirm the feature ADR/business flow and resolve only decisions that truly block implementation. Keep authorization, concurrency, rate limits, observability, and other engineering safeguards proportional and inside the relevant implementation/verification work; do not let generic technical concerns replace feature delivery in the Story or Definition of Done.
13. **Parallelize by product domain, not by splitting one feature across the whole team.** Plan SupportPlan for one focused owner while independent members advance appointment, specialist/admin, Journal/AI, and other implementation-ready domains. Cross-domain dependencies must be explicit, but they are not a reason to serialize unrelated feature work.
...
The owner selected NestJS for Journal/AI, Realtime, and Content/Notification on 2026-08-22. ADR 0006 supersedes ADR 0003 and fixes NestJS 11, strict TypeScript, feature modules, local-only `dotenv`, and Zod configuration validation while preserving contract-first service boundaries.

...
| --- | --- | --- |
| Registration/verification | Register USER or SPECIALIST, verify email; specialist begins `PENDING_VERIFICATION` | Duplicate normalized email conflicts; passwords are hashed; OTP/token expires, is one-use and rate-limited; account/outbox commit together |
...
- Keep JWT signing, hashing, Redis OTP, Brevo-trigger integration, Kafka, and persistence behind application ports. Identity has no outbound business REST dependency.
...
- [ ] ID-01 Define account lifecycle, role matrix, token/session and recovery policies; resolve session revocation ambiguity.
- [ ] ID-02 Publish registration, verification, login, refresh, logout, recovery and account-admin OpenAPI.
- [ ] ID-03 Publish account lifecycle, deletion and minimized audit event schemas.
- [ ] ID-04 Add owner Liquibase migrations, data dictionary entries, constraints and query indexes.
- [ ] ID-05 Implement registration/verification and safe delivery request integration.
- [ ] ID-06 Implement login, refresh rotation/replay detection and logout.

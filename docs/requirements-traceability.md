...
| 62–73, 148–149 | chat/in-person booking, transitions, evidence, history and read-only admin monitoring | Consultation/Billing | 60-minute slot; bounded request/response deadlines; immutable mode/location snapshot; credit and earning transitions locally transactional |
...
- **User:** assessment/journal → optional exact-revision AI Companion analysis → screening/safety → SupportEvaluation → system-proposed SupportPlan → explicit activation → specialist discovery → premium/payment/credit → request a 60-minute chat or in-person slot → review/edit/share appointment brief → bounded session → confirm delivery or report issue → read summary/review/follow-up.
- **Specialist:** register → complete public profile → admin approval → publish chat/in-person availability → accept/reject bounded requests → read only the approved appointment brief → consult/check in during the scheduled window → record delivery and user-visible summary → earnings/provider-payout projection → reviews.
...
- 2026-09-13: `MB-CONSULTATION-FLOW-001` enables 60-minute `IN_APP_CHAT` and `IN_PERSON` appointments with active practice locations, bounded request/response deadlines, explicit cancellation/credit outcomes, replacement-request rescheduling, evidence-based session completion, appointment-scoped user-approved `ConsultationBrief`, user-visible `SessionSummary`, non-clinical specialist discovery, and stable approval/suspension reasons. Admin appointment monitoring is read-only and excludes raw journal/answer/private-chat data. This approval does not implement contracts, migrations, frontend, Realtime, Care sharing, or Journal/AI brief runtime. See [ADR 0014](adr/0014-appointment-specialist-and-consultation-continuity.md) and [Consultation and specialist policy v1](policies/consultation-specialist-policy-v1.md).

...
3. Keep behavior with its tests, a material persistence migration with its canonical logical model, data dictionary, and mapping tests, and generated output with its source contract. A commit must build or validate independently and must not rely on a later commit to repair a known broken state.

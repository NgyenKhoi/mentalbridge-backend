...
| `IDENTITY_PASSWORD_RECOVERY_URL` | When delivery is enabled | Frontend reset-password URL receiving the one-time recovery challenge | `http://localhost:3000/reset-password` |
| `IDENTITY_LOCAL_VERIFICATION_DIRECTORY` | In `local-file` mode | Ignored local directory receiving purpose-specific verification/recovery URL files for synthetic accounts | `.local/identity-verification` |
...
The Brevo adapter sends verification and password-recovery URLs only for eligible accounts after the owning transaction commits. Provider failure is logged with safe identifiers only and does not change the endpoint's generic response. Do not commit or share the `.env`, and use a development provider key rather than a production credential.
...
Each eligible delivery then writes only its frontend URL to `.local/identity-verification/<accountId>.verification-url` or `<accountId>.password-recovery-url`. The ignored directory may contain an active challenge, so use synthetic addresses, do not publish its files, and delete them after testing. Startup rejects this fallback with no profile, with `prod`, or with `prod` combined with `dev`.

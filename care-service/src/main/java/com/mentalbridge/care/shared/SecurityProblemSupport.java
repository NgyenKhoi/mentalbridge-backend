...
public class SecurityProblemSupport implements AuthenticationEntryPoint, AccessDeniedHandler {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response,
		...
		write(response, request, 403, "CARE_USER_REQUIRED", "A USER account is required");
	}
	...
	private void write(HttpServletResponse response, HttpServletRequest request, int status, String code, String title)
		...
		objectMapper.writeValue(response.getOutputStream(), Map.of("type", "/problems/" + code.toLowerCase().replace('_', '-'),

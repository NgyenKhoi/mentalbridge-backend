...
@RestController
@RequestMapping("/api/v2/support-evaluations")
@Validated
public class SupportEvaluationV2Controller {

	private final SupportEvaluationV2Service evaluations;

	public SupportEvaluationV2Controller(SupportEvaluationV2Service evaluations) {
		this.evaluations = evaluations;
	}

	@PostMapping
	ResponseEntity<SupportEvaluationV2Service.EvaluationView> evaluate(@AuthenticationPrincipal Jwt jwt,
		...
		var result = evaluations.evaluate(subject(jwt), key, correlationId == null ? UUID.randomUUID() : correlationId,
				new SupportEvaluationV2Service.EvaluationCommand(request.phq9AssessmentId(), request.gad7AssessmentId()));
		return ResponseEntity.created(URI.create("/api/v2/support-evaluations/" + result.supportEvaluationId())).body(result);
	}

	@GetMapping("/{supportEvaluationId}")
	SupportEvaluationV2Service.EvaluationView get(@AuthenticationPrincipal Jwt jwt,

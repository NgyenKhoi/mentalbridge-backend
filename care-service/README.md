...
MB-89 implements the deterministic PHQ-9 runtime. MB-178 adds the backend-owned, versioned privacy disclosure (currently `privacy-capstone-v2`), append-only `PRIVACY_POLICY` decisions, optimistic Care profile replacement, cursor-based owned history, and explicit reassessment as a new immutable submission. Anonymous activity extends the 30-minute inactivity deadline only up to the two-hour absolute maximum. Registered history is authorized only for synthetic/test/demo use; production retention, deletion, export, specialist sharing, AI/research/marketing consent, and automatic clinical reminders remain unavailable rather than being invented.
